        ...
        if (body.isNotBlank()){
            ...
            if(!result.games.isNullOrEmpty() || !result.add_ons.isNullOrEmpty() || !result.add_ons.isNullOrEmpty()){
                    ...
                    CoroutineScope(SupervisorJob()).launch{
                        db.runInTransaction {
                            ...
                            result.games.forEach {
                                ...
                                dbGame.insert(GameTableBean(gameId,
                                    it.id,it.name,it.player_min,it.player_max,it.playing_time,
                                    gameDifficulty,it.bgg_link,it.age,it.buying_price, it.stock,
                                    it.max_time, it.external_img, it.picture, it.by_player, false))
                            }
                            result?.deleted_games?.forEach {
                                val gameInDb = dbGame.getByServerId(it.toLong())
                                if(gameInDb.isNotEmpty()) {
                                    dbGame.deleteOne(gameInDb[0].id)
                                }
                            }
                        }
                    }
                    ...
                    CoroutineScope(SupervisorJob()).launch {
                        db.runInTransaction {
                            ...
                            result.add_ons.forEach {
                                ...
                                it.difficulty?.run{
                                    val listGameDifficulty = db.difficultyDao().getByName(this)
                                    if (listGameDifficulty.isNotEmpty()) gameDifficulty = listGameDifficulty[0].id

                                }
                                var gameId: Long? = null
                                it.game?.run{
                                    val listGame = dbGame.getByName(this)
                                    if (listGame.isNotEmpty()) gameId = listGame[0].gameId
                            ...
                            result?.deleted_add_ons?.forEach {
                                val gameInDb = dbGame.getByServerId(it.toLong())
                                if (gameInDb.isNotEmpty()) dbGame.deleteOne(gameInDb[0].id)

                            }
                        }
                    }

                    allAddOns.removeIf { g -> this.any{g.name == it.name} || result.deleted_add_ons?.contains(g.id) == true }
                ...
                result.multi_add_ons?.run{
                    ...
                    allMultiAddOns.removeIf { g -> this.any{g.name == it.name} || result.deleted_multi_add_ons?.contains(g.id) == true }
